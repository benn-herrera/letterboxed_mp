...
class Generator:
    ...
    def generate_ctx(self, *, hdr: Optional[Path]=None, src: Optional[Path]=None) -> Tuple[Optional[GenCtx], Optional[GenCtx]]:
        if hdr and not self.generates_header:
            raise Exception(f"{self.name} does not generate a header file but hdr path was specified")
        ...
        if src and not self.generates_source:
            raise Exception(f"{self.name} does not generate a source file but src path was specified")
